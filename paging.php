...
if ($total_pages > 1) {
    ...
    if ($page > 1) {
        echo "<li><a href='?' title='Переход к первой странице'>";
        echo "Первая";
        echo "</a></li>";
    }
    ...
    for ($x = $initial_num; $x < $limit_num; $x++) {
        ...
        if (($x > 0) && ($x <= $total_pages)) { // Вводим доп. проверки, чтобы не получить страницу -1 и тд.
            ...
            // до и после работает else
                ...
                echo "<li><a href='?page=$x'>$x</a></li>"; //последующие и предыдущие
            }
        }
    }
...
// ссылка на последнюю страницу
    if ($page < $total_pages) {
        echo "<li><a href='?page={$total_pages}' title='Переход к последней странице'>";
        echo "Последняя";
        echo "</a></li>";
    }
}
